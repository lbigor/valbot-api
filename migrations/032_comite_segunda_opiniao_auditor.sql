-- 032 — Comitê como SEGUNDA OPINIÃO; Auditor é o decisor.
--
-- Regra de negócio (Igor, dura):
--   • O ④ Auditor é o DECISOR (A/R + justificativa).
--   • O ③ Comitê de IA é apenas SEGUNDA OPINIÃO (parecer/subsídio). NÃO encerra
--     o caso nem o filtra da fila do auditor.
--
-- Consequências sobre o STAGE de divergência da v_exams_overview:
--   (1) O Auditor SÓ recebe OS que JÁ passaram pelo Comitê. Divergência ainda
--       SEM comitê → fica em 'comite' (aguardando análise do comitê), NÃO vai
--       ao auditor.
--   (2) TODAS as divergências que passaram pelo comitê vão ao Auditor —
--       INCLUSIVE aquelas em que o comitê concordou com o examinador. O parecer
--       do comitê (resultado_comite/recomendacao/tipo_divergencia_pos_comite)
--       vai JUNTO como subsídio, mas o auditor decide. Antes, o filtro
--       tipo_divergencia_pos_comite='sem_divergencia' (no server.py) ESCONDIA
--       esses casos da fila — ERRADO; removido nesta frente.
--
-- O que muda na CASE de stage (vs migration 031):
--   ANTES:
...
--     ELSE 'concluido'   -- consenso (não divergente) não passa por comitê/auditor
--
...
--
-- TUDO o mais da 031 é preservado SEM regressão: as colunas/derivados (027+028+
-- 031), os buckets aguardando/aguardando_oficial por buscas_oficial, processando,
-- falhou, fora_escopo, concluido, e a v_exams_metrics (recriada pelo CASCADE).
--
...
SELECT
  ...
  CASE
    ...
    -- ── Comitê = SEGUNDA OPINIÃO; Auditor = decisor (032) ─────────────────────
    -- Divergência SEM comitê → fica no comitê (segunda opinião pendente); NÃO vai
    -- ao auditor. Divergência COM comitê → vai ao Auditor, INDEPENDENTE do parecer
    -- do comitê (inclusive quando concordou com o examinador). Consenso (não
    -- divergente) não passa por comitê nem auditor.
    WHEN base.divergente AND NOT base.comite_concluido                       THEN 'comite'
    WHEN base.divergente AND base.comite_concluido                           THEN 'auditoria'
    ELSE 'concluido'
  END AS stage
