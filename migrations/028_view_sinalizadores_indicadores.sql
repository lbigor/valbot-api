-- 028 — Sinalizadores oficiais na view + agregados do indicador de recebidos.
--
-- (1) Sinalizadores na view oficial: video_ok e tem_anotacoes passam a ser
--     campos CANÔNICOS da v_exams_overview (antes video_ok era derivado ad-hoc
--     no /api/os). Assim todo sinalizador sai da fonte única.
-- (2) Indicadores: v_exams_metrics ganha, por dia, quantos recebidos já têm
--     resultado oficial (A/R), quantos aguardam e quantos já têm anotações.
--
-- Atômica + idempotente. Só ADICIONA colunas (retrocompatível). Mantém tudo da 027.
...
COMMENT ON VIEW v_exams_overview IS
  ...
  'resultado_calculado/divergente/comite_concluido/conduta_inadequada/stage) + '
  'sinalizadores oficiais (video_ok, tem_anotacoes). Todas as telas leem daqui.';

-- ── Indicadores: variação do mesmo core, agora com recebidos × oficial × anotações ──
...
COMMENT ON VIEW v_exams_metrics IS
  ...
  'Recebidos × resultado oficial (A/R) × aguardando × anotações.';
